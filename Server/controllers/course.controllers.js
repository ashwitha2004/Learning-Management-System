...
export const getAllCourse = asyncHandler(async (req, res, next)=>{
    try {
        // Default limit is generous (larger than the current catalogue) so existing
        // callers that don't pass page/limit see unchanged behaviour; page/limit are
        // still available for when the catalogue grows.
...
export const addLectureToCourseById= asyncHandler(async(req, res, next )=>{
    ...
    if(!req.file){
        return next(
            new AppError('Lecture video is required', 400)
        )
    }
