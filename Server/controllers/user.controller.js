...
// "Remember me" keeps the session alive for 30 days instead of the normal 7
...
export const register=asyncHandler(async(req,res,next)=>{
    ...
    if(req.file){
        ...
        try {
            ...
            console.error('Avatar upload failed, keeping default avatar:', e.message || e);
        }
    }
...
export const login=asyncHandler(async (req,res,next)=>{
    ...
    try {
        ...
        const token = rememberMe
            ? await user.generateJWTToken(REMEMBER_ME_EXPIRY)
            : await user.generateJWTToken();
        user.password= undefined;

        const activeCookieOptions = rememberMe
            ? { ...cookieOptions, maxAge: REMEMBER_ME_MAX_AGE }
            : cookieOptions;

        res.cookie('token', token, { ...activeCookieOptions, sameSite: 'None' });
