...
export const getuserData=createAsyncThunk("/user/details", async ()=>{
    try {
        ...
        // a failed session check on load just means "not logged in" — not
        // worth surfacing as an error toast to every anonymous visitor
        return null;
    }
})
    ...
    extraReducers:(builder)=>{
        ...
        .addCase(getuserData.fulfilled, (state, action)=>{
            if(!action?.payload?.user){
                // no valid session on the server — don't trust stale localStorage
                localStorage.clear();
                state.isLoggedIn=false;
                state.data={};
                state.role="";
